...
import com.tencent.xlab.infinixcamera2.utils.ImageSaver;
import com.tencent.xlab.infinixcamera2.view.AutoFitTextureView;
...
public class Camera2RawActivity extends AppCompatActivity implements View.OnClickListener {
    /**
    ...
    /**
     * An {@link OrientationEventListener} used to determine when device rotation
     * has occurred.
     * This is mainly necessary for when the device is rotated by 180 degrees, in
     * which case
     * onCreate or onConfigurationChanged is not called as the view dimensions
     * remain the same,
     * but the orientation of the has changed, and thus the preview rotation must be
     * updated.
     */
    ...
    /**
     * An additional thread for running tasks that shouldn't block the UI. This is
     * used for all
    ...
    /**
     * A counter for tracking corresponding {@link CaptureRequest}s and
     * {@link CaptureResult}s
    ...
    //
    // The following state is used across both the UI and background threads.
    // Methods with "Locked"
    ...
    /**
     * A reference counted holder wrapping the {@link ImageReader} that handles JPEG
     * image
     * captures. This is used to allow us to clean up the {@link ImageReader} when
     * all background
     * tasks using its {@link Image}s have completed.
     */
    private ImageSaver.RefCountedAutoCloseable<ImageReader> mJpegImageReader;

    /**
     * A reference counted holder wrapping the {@link ImageReader} that handles RAW
     * image captures.
     * This is used to allow us to clean up the {@link ImageReader} when all
     * background tasks using
     * its {@link Image}s have completed.
     */
    private ImageSaver.RefCountedAutoCloseable<ImageReader> mRawImageReader;
    ...
    /**
     * Request ID to {@link ImageSaver.ImageSaverBuilder} mapping for in-progress
     * JPEG captures.
     */
    ...
    /**
     * Timer to use with pre-capture sequence to ensure a timely capture if 3A
     * convergence is
     * taking too long.
     */
    ...
    /**
     * {@link CameraDevice.StateCallback} is called when the currently active
     * {@link CameraDevice}
     * changes its state.
     */
    private final CameraDevice.StateCallback mStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onError(CameraDevice cameraDevice, int error) {
            ...
            Activity activity = Camera2RawActivity.this;
            if (null != activity) {
                activity.finish();
            }
        }
    ...
    /**
     * A {@link CameraCaptureSession.CaptureCallback} that handles events for the
     * preview and
    ...
    /**
     * A {@link CameraCaptureSession.CaptureCallback} that handles the still JPEG
     * and RAW capture
     * request.
     */
    private final CameraCaptureSession.CaptureCallback mCaptureCallback = new CameraCaptureSession.CaptureCallback() {
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
            ...
            // Look up the ImageSaverBuilder for this request and update it with the file
            // name
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            // Look up the ImageSaverBuilder for this request and update it with the
            // CaptureResult
            synchronized (mCameraStateLock) {
                jpegBuilder = mJpegResultQueue.get(requestId);
                rawBuilder = mRawResultQueue.get(requestId);
                ...
                // If we have all the results necessary, save the image to a file in the
                // background.
    ...
    protected void onResume() {
        ...
        // we should
        // configure the preview bounds here (otherwise, we wait until the surface is
        // ready in
    ...
    private void initView() {
        ...
        findViewById(R.id.btn_expTime).setOnClickListener(this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_iso:
                new XPopup.Builder(Camera2RawActivity.this)
                        .asCustom(new RawIsoListDrawerPopupView(Camera2RawActivity.this))
                        .show();
                break;
            case R.id.btn_expTime:
                new XPopup.Builder(Camera2RawActivity.this)
                        .asCustom(new ExpListDrawerPopupView(Camera2RawActivity.this))
                        .show();
                break;
        }
    }
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        synchronized (mCameraStateLock) {
            ...
            // Swap the view dimensions for calculation as needed if they are rotated
            // relative to
            // the sensor.
            ...
            // Find the best preview size for these view dimensions and configured JPEG
            // size.
            ...
            // Find rotation of device in degrees (reverse device orientation for
            // front-facing
            // cameras).
            ...
            // Initially, output stream images from the Camera2 API will be rotated to the
            // native
            // device orientation from the sensor's orientation, and the TextureView will
            // default to
            // scaling these buffers to fill it's view bounds. If the aspect ratios and
            // relative
            // orientations are correct, this is fine.
            //
            // However, if the device orientation has been rotated relative to its native
            // orientation so that the TextureView's dimensions are swapped relative to the
            // native device orientation, we must do the following to ensure the output
            // stream
            // images are not incorrectly scaled by the TextureView:
            // - Undo the scale-to-fill from the output buffer's dimensions (i.e. its
            // dimensions
    ...
    /**
     * Configure the given {@link CaptureRequest.Builder} to use auto-focus,
     * auto-exposure, and
    ...
    private void setup3AControlsLocked(CaptureRequest.Builder builder) {
        ...
        // If MINIMUM_FOCUS_DISTANCE is 0, lens is fixed-focus and we need to skip the
        // AF run.
        mNoAFRun = (minFocusDist == null || minFocusDist == 0);

        if (!mNoAFRun) {
            // If there is a "continuous picture" mode available, use it, otherwise default
            // to AUTO.
        ...
        // If there is an auto-magical flash control mode available, use it, otherwise
        // default to
    ...
    private void captureStillPictureLocked() {
        try {
            final Activity activity = Camera2RawActivity.this;
            ...
            // Create an ImageSaverBuilder in which to collect results, and add it to the
            // queue
            // of active requests.
    ...
    /**
     * Rotation need to transform from the camera sensor orientation to the device's
     * current
     * orientation.
     *
     * @param c                 the {@link CameraCharacteristics} to query for the
     *                          camera sensor
     *                          orientation.
     * @param deviceOrientation the current device orientation relative to the
     *                          native device
     *                          orientation.
     * @return the total rotation from the sensor orientation to the current device
     *         orientation.
     */
    ...
    /**
     * Called after a RAW/JPEG capture has completed; resets the AF trigger state
     * for the
     * pre-capture sequence.
     * <p/>
     * Call this only with {@link #mCameraStateLock} held.
    ...
    private void openCamera() {
        ...
        try {
            ...
            // Attempt to open the camera. mStateCallback will be called on the background
            // handler's
    ...
    /**
     * Sets up state related to camera that is needed before opening a
     * {@link CameraDevice}.
     */
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                synchronized (mCameraStateLock) {
                    ...
                    if (mJpegImageReader == null || mJpegImageReader.getAndRetain() == null) {
                        mJpegImageReader = new ImageSaver.RefCountedAutoCloseable<>(
                    ...
                    if (mRawImageReader == null || mRawImageReader.getAndRetain() == null) {
                        mRawImageReader = new ImageSaver.RefCountedAutoCloseable<>(
    ...
    private void closeCamera() {
        try {
            ...
            synchronized (mCameraStateLock) {
                ...
                // Note: After calling this, the ImageReaders will be closed after any
                // background
     ...
     * smallest one that
     * is at least as large as the respective texture view size, and that is at most
     * as large as the
     * respective max size, and whose aspect ratio matches with the specified value.
     * If such size
     * doesn't exist, choose the largest one that is at most as large as the
     * respective max size,
     ...
     * in our state
     * machine that waits for auto-focus to finish, ending in a "locked" state where
     * the lens is no
     * longer moving, waits for auto-exposure to choose a good exposure value, and
     * waits for
     * auto-white-balance to converge.
     */
    private void takePicture() {
        Log.e("Take", "takePicture");
        synchronized (mCameraStateLock) {
            mPendingUserCaptures++;
            // If we already triggered a pre-capture sequence, or are in a state where we
            // cannot
            ...
            try {
                // Trigger an auto-focus run if camera is capable. If the camera is already
                // focused,
    ...
    private void showToast(String text) {
        // We show a Toast by sending request message to mMessageHandler. This makes
        // sure that the
    ...
    /**
     * Retrieve the next {@link Image} from a reference counted {@link ImageReader},
     * retaining
     * that {@link ImageReader} until that {@link Image} is no longer in use, and
     * set this
     * {@link Image} as the result for the next request in the queue of pending
     * requests. If
     * all necessary information is available, begin saving the image to a file in a
     * background
     * thread.
     *
     * @param pendingQueue the currently active requests.
     * @param reader       a reference counted wrapper containing an
     *                     {@link ImageReader} from which
     *                     to acquire an image.
     */
    private void dequeueAndSaveImage(TreeMap<Integer, ImageSaver.ImageSaverBuilder> pendingQueue,
            ImageSaver.RefCountedAutoCloseable<ImageReader> reader) {
        synchronized (mCameraStateLock) {
    ...
    /**
     * If the given request has been completed, remove it from the queue of active
     * requests and
     * send an {@link ImageSaver} with the results from this request to a background
     * thread to
     * save a file.
     * <p/>
     * Call this only with {@link #mCameraStateLock} held.
    ...
    /**
     * Generate a string containing a formatted timestamp with the current date and
     * time.
     *
    ...
    /**
     * A dialog fragment for displaying non-recoverable errors; this
     * {@ling Activity} will be
    ...
    class RawIsoListDrawerPopupView extends DrawerPopupView {
        RecyclerView recyclerView;

        public RawIsoListDrawerPopupView(@NonNull Context context) {
            super(context);
        }
        ...
        protected void onCreate() {
            ...
            addIsoData();
            final EasyAdapter<String> commonAdapter = new EasyAdapter<String>(data,
                ...
                protected void bind(ViewHolder viewHolder, final String s, int i) {
                    ...
                    viewHolder.getView(android.R.id.text1).setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View v) {
                            Camera2RawActivity.this.setIsoChange(iso);
                        }
                    });
                }
    ...
    class ExpListDrawerPopupView extends DrawerPopupView {
        RecyclerView recyclerView;
            ...
            final EasyAdapter<Long> commonAdapter = new EasyAdapter<Long>(data, android.R.layout.simple_list_item_1) {
                ...
                protected void bind(ViewHolder viewHolder, final Long s, int i) {
                    ...
                    viewHolder.getView(android.R.id.text1).setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Camera2RawActivity.this.setExposureTime(s * 1000);
                            Camera2RawActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    tv_expTime.setText(s * 1000 + "");
                                }
                            });
                        }
                    });
